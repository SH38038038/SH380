...
loginController : 로그인 시 비밀번호 검증을 위해 PasswordEncoder를 사용, 이때 SecurityConfig 주입
   ↓
securityConfig 
   ↓
customOAuth2SuccessHandler : SecurityConfig 내부에서 @Bean
   ↓
securityConfig (다시 올라감)

 
해결 방법
소셜로그인만 사용할 것이기 때문에 loginController에서 PasswordEncoder 주입 삭제.
SecurityConfig 내부에서 customOAuth2SuccessHandler  @Bean 등록 후 SecurityFilterChain에서 customOAuth2SuccessHandler를 사용.
CustomOAuth2SuccessHandler에서는 TokenService, UserRepository, ObjectMapper, OAuth2AuthorizedClientService를 주입받아 사용. Config 자체에는 의존하지 않음. 
